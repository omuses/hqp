/*---------------------------------------------------------------------------- 
 ...
 Revision: $Id: taylor.c,v 1.1 2004/10/13 14:18:12 e_arnold Exp $
...
/*--------------------------------------------------------------------------*/
double summand(int p, int d, int* jm, int* km, int order_im, int order_km, long binomiZ)
...
void coeff(int p, int d, struct item* coeff_list)
{
  ...
  for (index_coeff_list = 0; 1; index_coeff_list++)
  {  /* travers coeff_list, i.e. create all j with |j| = d. */
    ptr = NULL;
    for (order_im=1; order_im<=d; order_im++)
    {  /* travers all orders from 1 to d for i */
      im[p-1]=0;
      im[0] = order_im;
      while (1)
      {   /* create all i with order order_im. */
        sum = 0;
        binomiZ = 1;
