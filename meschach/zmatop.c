...
static	char	rcsid[] = "$Id: zmatop.c,v 1.2 2002/12/09 10:57:47 e_arnold Exp $";
...
ZMAT	*mat1,*mat2,*out;
{
	...
	m_error(E_NULL,"zm_add");
    if ( mat1->m != mat2->m || mat1->n != mat2->n )
	m_error(E_SIZES,"zm_add");
...
ZMAT	*mat1,*mat2,*out;
{
    ...
    if ( mat1==ZMNULL || mat2==ZMNULL )
	m_error(E_NULL,"zm_sub");
    if ( mat1->m != mat2->m || mat1->n != mat2->n )
	m_error(E_SIZES,"zm_sub");
    if ( out==ZMNULL || out->m != mat1->m || out->n != mat1->n )
	out = zm_resize(out,mat1->m,mat1->n);
    m = mat1->m;	n = mat1->n;
    for ( i=0; i<m; i++ )
...
ZMAT	*A,*B,*OUT;
{
	...
	m_error(E_NULL,"zm_mlt");
    if ( A->n != B->m )
	m_error(E_SIZES,"zm_mlt");
    if ( A == OUT || B == OUT )
	m_error(E_INSITU,"zm_mlt");
    m = A->m;	n = A->n;	p = B->n;
    A_v = A->me;		B_v = B->me;
...
ZMAT	*A, *B, *OUT;
{
    ...
    if ( ! A || ! B )
	m_error(E_NULL,"zmma_mlt");
    if ( A == OUT || B == OUT )
	m_error(E_INSITU,"zmma_mlt");
    if ( A->n != B->n )
	m_error(E_SIZES,"zmma_mlt");
...
ZMAT	*A, *B, *OUT;
{
    ...
    if ( ! A || ! B )
	m_error(E_NULL,"zmam_mlt");
    if ( A == OUT || B == OUT )
	m_error(E_INSITU,"zmam_mlt");
    if ( A->m != B->m )
	m_error(E_SIZES,"zmam_mlt");
...
ZVEC	*b,*out;
{
	...
	m_error(E_NULL,"zmv_mlt");
    if ( A->n != b->dim )
	m_error(E_SIZES,"zmv_mlt");
    if ( b == out )
	m_error(E_INSITU,"zmv_mlt");
...
ZVEC	*b,*out;
{
    ...
    if ( A==ZMNULL || b==ZVNULL )
	m_error(E_NULL,"zvm_mlt");
    if ( A->m != b->dim )
	m_error(E_SIZES,"zvm_mlt");
    if ( b == out )
	m_error(E_INSITU,"zvm_mlt");
...
ZMAT	*in, *out;
{
    ...
    if ( in == ZMNULL )
	m_error(E_NULL,"zm_adjoint");
    if ( in == out && in->n != in->m )
	m_error(E_INSITU2,"zm_adjoint");
    in_situ = ( in == out );
...
complex	s;
{
    ...
    if ( ! A1 || ! A2 )
	m_error(E_NULL,"mz_mltadd");
    if ( A1->m != A2->m || A1->n != A2->n )
	m_error(E_SIZES,"mz_mltadd");
    ...
    m_tracecatch(out = zm_copy(A1,out),"mz_mltadd");
...
complex	alpha;
{
    ...
    if ( ! v1 || ! v2 || ! A )
	m_error(E_NULL,"zmv_mltadd");
    if ( out == v2 )
	m_error(E_INSITU,"zmv_mltadd");
    if ( v1->dim != A->m || v2->dim != A-> n )
	m_error(E_SIZES,"zmv_mltadd");
    
    m_tracecatch(out = zv_copy(v1,out),"zmv_mltadd");
...
complex	alpha;
{
    ...
    if ( ! v1 || ! v2 || ! A )
	m_error(E_NULL,"zvm_mltadd");
    if ( v2 == out )
	m_error(E_INSITU,"zvm_mltadd");
    if ( v1->dim != A->n || A->m != v2->dim )
	m_error(E_SIZES,"zvm_mltadd");
    
    m_tracecatch(out = zv_copy(v1,out),"zvm_mltadd");
...
ZVEC	*vec;
{
	...
	if ( mat==ZMNULL )
		m_error(E_NULL,"zget_col");
	if ( col < 0 || col >= mat->n )
		m_error(E_RANGE,"zget_col");
...
ZVEC	*vec;
{
	...
	if ( mat==ZMNULL )
		m_error(E_NULL,"zget_row");
	if ( row < 0 || row >= mat->m )
		m_error(E_RANGE,"zget_row");
...
int	col;
{
	...
	if ( mat==ZMNULL || vec==ZVNULL )
		m_error(E_NULL,"zset_col");
	if ( col < 0 || col >= mat->n )
		m_error(E_RANGE,"zset_col");
...
int	row;
{
	...
	if ( mat==ZMNULL || vec==ZVNULL )
		m_error(E_NULL,"zset_row");
	if ( row < 0 || row >= mat->m )
		m_error(E_RANGE,"zset_row");
	lim = min(mat->n,vec->dim);
	/* for ( j=j0; j<lim; j++ ) */
