...
VEC	*diag, *beta;
{
	...
	for ( k = 0; k < limit; k++ )
	{
		...
		/* rf, 10/27/00: check numerical overflow for beta 
		   and do the same as hhvec does if norm(tmp1) is zero.
		   This error can't be treated in hhvec, as the overflow
		   may not happen there due to use of registers!!!
		*/
		if ( !(beta->ve[k] < HUGE_VAL) )
		  {
		    beta->ve[k] = 0.0;
		  }
